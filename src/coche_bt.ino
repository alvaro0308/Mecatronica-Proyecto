...
AccelStepper stepper1(FULLSTEP, motor1_Pin1, motor1_Pin3, motor1_Pin2,
                      motor1_Pin4);
AccelStepper stepper2(FULLSTEP, motor2_Pin1, motor2_Pin3, motor2_Pin2,
                      motor2_Pin4);
...
void callback_ultrasonidos() {
  ...
  duracion = pulseIn(
      echo, HIGH);  // Medimos el tiempo que ha tardado en llegar el sonido
  distancia =
      (duracion * 0.034) / 2;  // Calculamos la distancia multiplicando por la
                               // velocidad del sonido y diviendo entre 2
}

void setup() {
  Serial.begin(
      9600);  // Inicio del puerto serial para comunicacion con el bluetooth
